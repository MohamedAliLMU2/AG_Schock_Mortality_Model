...
# Disclaimer Text
st.markdown("""
    This website and its pre-trained models are intended **solely for scientific research** and **prospective validation**.  
    They are **not** designed, approved, or intended for use in:
    - Clinical decision-making  
    - Patient diagnosis  
    - Treatment planning  
    - Any other real-world medical applications  
...
agreed = st.checkbox("I have read, understood, and accept the terms of this disclaimer.")

# Continue Button (Disabled until checkbox is selected)
if agreed:
    if st.button("Continue"):
        st.session_state['agreed'] = True
        st.success("You have accepted the terms. Proceeding...")
else:
    st.warning("You must accept the disclaimer before continuing.")


if 'agreed' in st.session_state:

    
    
    st.markdown(
        """
        This website and its pre-trained models are designed exclusively for scientific research and should be used only for the prospective validation of the models. Any application of this tool for predicting patient mortality in real-world clinical settings or for guiding medical interventions is strictly prohibited. The authors assume no responsibility for any decisions, actions, or consequences resulting from the use or misuse of this tool. Users acknowledge that this tool is not a substitute for professional medical judgment and should not be relied upon for clinical decision-making.    
        ...
        ### Interested in learning more details about our models?
    ...
    if uploaded_file is not None:
        ...
        class model_24h_mortality:
            def __init__(self, 
                ...
                self.models = {
                    1: {'model': p_model_24h, 'outlier_model': s_model_24h, 'scaler': scaler_24h},
                    2: {'model': p_model_2d, 'outlier_model': s_model_2d, 'scaler': scaler_2d},
                    3: {'model': p_model_3d, 'outlier_model': s_model_3d, 'scaler': scaler_3d},
                    4: {'model': p_model_4d, 'outlier_model': s_model_4d, 'scaler': scaler_4d},
                    5: {'model': p_model_5d, 'outlier_model': s_model_5d, 'scaler': scaler_5d},
                    6: {'model': p_model_6d, 'outlier_model': s_model_6d, 'scaler': scaler_6d},
                    7: {'model': p_model_7d, 'outlier_model': s_model_7d, 'scaler': scaler_7d},
                    8: {'model': p_model_8d, 'outlier_model': s_model_8d, 'scaler': scaler_8d},
                    9: {'model': p_model_9d, 'outlier_model': s_model_9d, 'scaler': scaler_9d},
                    10: {'model': p_model_10d, 'outlier_model': s_model_10d, 'scaler': scaler_10d},
                    11: {'model': p_model_11d, 'outlier_model': s_model_11d, 'scaler': scaler_11d},
                    12: {'model': p_model_12d, 'outlier_model': s_model_12d, 'scaler': scaler_12d},
                    13: {'model': p_model_13d, 'outlier_model': s_model_13d, 'scaler': scaler_13d},
                    14: {'model': p_model_14d, 'outlier_model': s_model_14d, 'scaler': scaler_14d},
                    15: {'model': p_model_15d, 'outlier_model': s_model_15d, 'scaler': scaler_15d},
                    21: {'model': p_model_3w, 'outlier_model': s_model_3w, 'scaler': scaler_3w}
                }
            ...
            def predict(self, df_input):
                ...
                for day in range(1, 16):
                    ...
                    pred_proba = final_model.predict_proba(df_input)[0][1]#[:, 1]
        ...
        if User_Input == "Automatic input form pdf/excel":        
            ...
            if 'values' in st.session_state:
                ...
                if Input_modify == "yes": 
                    ...
                    edited_df = st.data_editor(weighted_means_df.T,  hide_index=False, )#use_container_width=True,num_rows="dynamic"
